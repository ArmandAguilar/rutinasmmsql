...
sql = 'SELECT [NumProyecto],[NomProyecto],ISNULL([NumMaestro],0),ISNULL([Dias de produccion],0),ISNULL([Trabajo por programar],0),ISNULL([Margen Actual],0),CONVERT(NVARCHAR(10),[PeriodoComparativo]) as PeriodoComparativo FROM [SAP].[dbo].[RV-ESTADOPROYECTOS-AA-Throughput] order by NumMaestro desc'
conn = pymssql.connect(host=hostMSSQL,user=userMSSQL,password=passMSSQL,database=dbMSSQL)
cur = conn.cursor()
cur.execute(sql)
for value in cur:
